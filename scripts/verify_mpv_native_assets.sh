...
verify_abi() {
  ...
  contains_string "$directory/libmpv.so" "android-dovi-fel"
  contains_string "$directory/libmpv.so" "WebHTV Android FEL: software enhancement-layer decoder"
  contains_string "$directory/libmpv.so" "WebHTV FEL GPU input: matched EL uploaded with active NLQ."
  contains_string "$directory/libmpv.so" "WebHTV FEL decoder queue:"
  contains_string "$directory/libmpv.so" "WebHTV FEL stats:"
  contains_string "$directory/libmpv.so" "WebHTV FEL fatal:"
  contains_string "$directory/libmpv.so" "WebHTV FEL GPU staging:"
  contains_string "$directory/libmpv.so" "WebHTV FEL GPU pool:"
  contains_string "$directory/libmpv.so" "WebHTV FEL status snapshot:"
  contains_string "$directory/libmpv.so" "WebHTV FEL pipeline diagnostics v=1 enabled;"
  ...
  contains_string "$directory/libmpv.so" "WebHTV FEL source release:"
  contains_string "$directory/libmpv.so" "WebHTV FEL map cost:"
  contains_string "$directory/libmpv.so" "WebHTV FEL GPU init:"
  contains_string "$directory/libmpv.so" "WebHTV FEL perf:"
  contains_string "$directory/libmpv.so" "WebHTV FEL perf stages:"
  contains_string "$directory/libmpv.so" "WebHTV FEL reuse:"
  contains_string "$directory/libmpv.so" "descriptor-content-cache=1"
  contains_string "$directory/libmpv.so" "latency-window:"
  ...
  contains_string "$directory/libmpv.so" "wait=async."
  ...
  contains_string "$directory/libmvcodec.so" "avs3_mediacodec"
  contains_string "$directory/libmvcodec.so" "WebHTV hardware AVS3 MediaCodec decoder:"
  contains_string "$directory/libmvcodec.so" "No hardware AVS3 MediaCodec decoder"
  contains_string "$directory/libmvcodec.so" "AVS3 MediaCodec requires an explicit hardware context"
  ...
  contains_string "$directory/libmvcodec.so" "WebHTV AV-DIAG codec-lookup v=1"
